...
%         size(x)
%         size(u)
%     if xmeasure(1)^2/a_safe^2+xmeasure(2)^2/b_safe^2+xmeasure(3)^2/c_safe^2>=0.99&& xmeasure(1)^2/a_sa^2+xmeasure(2)^2/b_sa^2+xmeasure(3)^2/c_sa^2-1<=0
%         fprintf('满足\n');
%     else
%         fprintf('whats hell');
%     end
...
%% computeOpenloopSolution/linearconstraints/nonlinearconstraints
...
function [x, t_intermediate, x_intermediate] = dynamic(system, T, t0, ...
    ...
    if ( strcmp(type, 'difference equation') )
        x = system(t0, x0, u, T);%这个重点关注，有关系统的
        x_intermediate = [x0; x];
...
%% system
%% runningcosts;terminalcosts;
%% constraints;terminalconstraints;linearconstraints
% nmpc是针对非线性动力学的还是什么？能不能处理非线性约束？
% 非线性动力学倒是比较好处理，但是非线性约束就比较难弄，他是将线性约束
% 加以反复迭代之后获得非线性约束是吗？但是有预测的过程吗？
% 就是将控制量反复叠加到动力学之后产生的最终预测时域的结果，得到的状态量对其加权是吗？
